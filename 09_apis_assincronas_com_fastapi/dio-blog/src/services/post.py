...
class PostService:
    ...
    async def read_all(self, published: bool, limit: int, skip: int = 0) -> list[Record]:
        query = posts.select().where(posts.columns.published == published).limit(limit).offset(skip)
    ...
    async def update(self, id: int, post: PostUpdateIn) -> Record:
        ...
        query = posts.update().where(posts.columns.id == id).values(**data)
    ...
    async def __get_by_id(self, id: int) -> Record:
        query = posts.select().where(posts.columns.id == id)
